...
function StrategyButtons({ onSeeStrategies }: { onSeeStrategies: () => void }) {
  return (
    <>
      <button
        type="button"
...
export function Hero() {
  ...
  return (
    <section
      ref={ref}
      className="mx-auto grid w-full max-w-6xl items-center gap-4 px-6 pb-4 pt-9 min-[860px]:grid-cols-[1.05fr_0.95fr] min-[860px]:gap-11 min-[860px]:pb-8"
    >
      <motion.div
        className="min-w-0"
      ...
      >
        ...
        {/* Both variants always render; only CSS visibility toggles at the
            min-[860px] breakpoint, so SSR and CSR paint identical DOM. */}
        <div className="hidden flex-wrap gap-3.5 min-[860px]:flex">
          <StrategyButtons onSeeStrategies={handleSeeStrategies} />
        </div>
      </motion.div>

      <QuickEstimate />
      <div className="flex flex-wrap gap-3.5 min-[860px]:hidden">
        <StrategyButtons onSeeStrategies={handleSeeStrategies} />
      </div>
    </section>
  );
